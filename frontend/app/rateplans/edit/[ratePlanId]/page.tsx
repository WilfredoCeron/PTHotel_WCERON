...
export default function EditRatePlanPage() {
  ...
  useEffect(() => {
    ...
    apiClient.get(`/rateplans/${ratePlanId}`).then(res => setForm(res.data)).catch(() => setError("No se pudo cargar la tarifa"));
  }, [ratePlanId]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      await apiClient.put(`/rateplans/${ratePlanId}`, {
        ...form,
  ...
  return (
    <form className="card p-4" onSubmit={handleSubmit}>
      <h3 className="mb-3">Editar Tarifa</h3>
      <div className="mb-2">
        <label>Hotel</label>
      ...
      <div className="mb-2">
        <label>Precio</label>
        <input name="price" type="number" min="0" value={form.price || 0} onChange={handleChange} className="form-control" required />
      </div>
      <div className="mb-2">
        <label>Fecha Inicio</label>
        <input name="startDate" type="date" value={form.startDate ? form.startDate.substring(0,10) : ""} onChange={handleChange} className="form-control" required />
      </div>
      <div className="mb-2">
        <label>Fecha Fin</label>
        <input name="endDate" type="date" value={form.endDate ? form.endDate.substring(0,10) : ""} onChange={handleChange} className="form-control" required />
      </div>
